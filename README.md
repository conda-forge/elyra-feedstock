...
Once the `conda-forge` channel has been enabled, `elyra` can be installed with `conda`:
...
It is possible to list all of the versions of `elyra` available on your platform with `conda`:
...
```
mamba search elyra --channel conda-forge
```
...
mamba repoquery search elyra --channel conda-forge

# List packages depending on `elyra`:
mamba repoquery whoneeds elyra --channel conda-forge

# List dependencies of `elyra`:
mamba repoquery depends elyra --channel conda-forge
```
